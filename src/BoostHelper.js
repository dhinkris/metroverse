...
import SelectedBoosts from './components/SelectedBoosts'
...
class BoostHelper extends React.Component {
    ...
    handleSearch = async (selected_boosts) => {
        const block_id = await search_blocks(selected_boosts)
        this.setState({selected_boosts: selected_boosts})
        this.setState({block_id: block_id })
    }
    handleClear=()=>{
        this.setState({ block_id:[] })
        this.setState({ selected_boosts:[]  })
    }
    render() {
        const block_id = this.state
        const selected_boosts = this.state
                ...
                <Box gridColumn="span 8">
                    <div style={{ display: 'flex' }}>
                        <div style={{ flexGrow: 1 }}>
                            <h5 style={{marginTop: "60px"}}> Disclaimer: This search feature is based on block info from 20000 to 52976. More info will be update hopefully by end of the day.
                            <br/><br/>
                            ...
                            <SelectedBoosts {...selected_boosts}/>
                            <Blocks  {...block_id} />
                            <ColorChips />
                        </div>
                    </div>
